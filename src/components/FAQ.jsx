      ...
      <div className="divide-y divide-line overflow-hidden rounded-[--radius-card] border border-line bg-white">
        {items.map((item) => (
          <details key={item.question} className="group">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-3.5 text-[14px] font-medium text-navy-900 transition-colors hover:bg-canvas sm:px-5">
            ...
            <div className="px-4 pb-4 text-[13px] leading-relaxed text-ink-700 sm:px-5">
              {item.answer}
            </div>
          </details>
        ))}
