...
/**
 * Horizontal scroll-snap track with measured paging dots.
 *
 * The caller supplies the items and their widths — this owns only the
 * scrolling, the measuring and the dots, so a row of banners and a row of
 * service cards page identically.
 *
 ...
 */
export default function CardSlider({ children, className = "", gapClass = "gap-3 sm:gap-4" }) {
  ...
  return (
    <div className={`min-w-0 ${className}`}>
      <ul
        ref={trackRef}
        onScroll={measure}
        className={`no-scrollbar flex snap-x snap-mandatory overflow-x-auto ${gapClass}`}
      >
        {children}
      </ul>
          ...
          {Array.from({ length: pages }, (_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-200 ${
                i === active ? "w-6 bg-brand-500" : "w-2 bg-line-strong hover:bg-ink-400"
              }`}
            />
          ))}
        </div>
