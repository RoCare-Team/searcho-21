        ...
        <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
          {(title || description) && (
            <div className="max-w-2xl">
              {title && <Heading className="text-xl font-semibold sm:text-[24px]">{title}</Heading>}
          ...
          {action && (
            <Link
              href={action.href}
              className="inline-flex shrink-0 items-center gap-1 text-[15.5px] font-medium text-brand-600 transition-colors hover:text-brand-700"
            >
              {action.label}
...
export function ChipLink({ href, children }) {
  return (
    <Link
      href={href}
      className="inline-flex items-center rounded-full border border-line bg-white px-3.5 py-1.5 text-[15.5px] text-ink-700 transition-colors hover:border-line-strong hover:text-brand-600"
    >
      {children}
    </Link>
  );
